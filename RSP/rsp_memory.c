...
int AllocateRspMemory (void) {
	if (RspRecompCode == NULL) {
		RspRecompCode = (BYTE*)VirtualAlloc(NULL, 0x02100004, MEM_RESERVE, PAGE_EXECUTE_READWRITE);
		RspRecompCode = (BYTE*)VirtualAlloc(RspRecompCode, 0x02100000, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	}

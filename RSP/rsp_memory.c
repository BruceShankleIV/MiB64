...
int AllocateRspMemory (void) {
	if (RspRecompCode == NULL) {
		RspRecompCode = (BYTE*)VirtualAlloc(NULL, 0x01a00004, MEM_RESERVE, PAGE_EXECUTE_READWRITE);
		RspRecompCode = (BYTE*)VirtualAlloc(RspRecompCode, 0x01a00000, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	}
